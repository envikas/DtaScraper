...
database_terms = ["my sql", "mysql","oracle", "sql server", "sqlserver", "db2", "sybase","hana", "postgresql", "postgre sql", "mariadb", "maria db", "teradata", "informic", "ingres", "simpledb"]
...
for j in range(0,len(resultList)):
    ...
    if (resultList[j][1] > 1 and resultList[j][0].lower() in database_terms):
        database_result += resultList[j][0] + "," + str(resultList[j][1]) + "\n"
...
with io.open("Database_Count_uk.csv", "w", encoding="utf-8") as f:
